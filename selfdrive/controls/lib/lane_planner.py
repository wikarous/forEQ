...
class LanePlanner():
  ...
  def update_d_poly(self, v_ego, sm):
    ...
    if curvature > 0.001: # left curve
        if Curv > 0.006:
          Curv = 0.006
        lean_offset = -0.04 - (Curv * 40) #move the car to right at left curve
    elif curvature < -0.001:   # right curve
        if Curv < -0.006:
          Curv = -0.006
        lean_offset = -0.00 + (Curv * 10) #move the car to right at right curve
    else:
        lean_offset = 0
    ...
    # only offset left and right lane lines; offsetting p_poly does not make sense
      ...
      self.l_poly[3] += CAMERA_OFFSET + (lean_offset / 2.0)
      self.r_poly[3] += CAMERA_OFFSET + (lean_offset / 2.0)
